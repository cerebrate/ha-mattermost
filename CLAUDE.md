...
This repo (`cerebrate/ha-mattermost`) is a fork of `zacs/ha-mattermost`, used for development rather than direct publishing. The HACS validation job's `<Validation issues>` and `<Validation topics>` checks fail here because this fork has Issues disabled and no repo topics set — that's expected and harmless on a dev fork (those checks would pass on the upstream repo, which does have both configured). Don't spend time trying to "fix" those two specific failures; every other CI job (style, lint, file structure, hassfest, and the pytest suite) should still be green and is the real signal to watch.
